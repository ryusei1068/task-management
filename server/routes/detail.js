...
router.get("/", (req, res) => {
  const {id} = req.query;
  detail.findOne({attributes : [ "id", "description" ], where : {taskid : id}})
      .then(detail => {
        if (detail === null) {
          res.sendStatus(204)
        } else {
...
router.put("/", (req, res) => {
  ...
  if (id === undefined) {
    insertItem(detail, {description : sanitizedDescription, taskid : taskid})
        .then(detail => {res.status(201).json(
                  {id : detail.id, description : detail.description})})
        .catch(() => { res.sendStatus(500); })
  } else {
    updateItem(detail, {description : sanitizedDescription}, {id : id})
        .then(() => {res.status(201).json(
                  {id : id, description : sanitizedDescription})})
        .catch(() => { res.sendStatus(500); })
  }
})
